...
export const SystemHUD: React.FC = () => {
  ...
  if (!isVisible) {
    return (
      <div className="fixed bottom-0 right-8 z-[40] hidden md:block">
        <button 
          onClick={() => setIsVisible(true)}
          className="bg-[#050505]/95 border border-white/10 border-b-0 rounded-t-lg px-4 py-1 text-[10px] font-mono text-primary-teal hover:text-white uppercase tracking-widest flex items-center gap-2 hover:shadow-[0_0_15px_rgba(0,240,255,0.2)] transition-all"
        >
          <i className="fa-solid fa-chevron-up"></i> System
        </button>
      </div>
    );
  }
    ...
    <div className="fixed bottom-0 left-0 w-full z-[40] bg-[#050505]/95 backdrop-blur-md border-t border-white/10 hidden md:block shadow-[0_-5px_20px_rgba(0,0,0,0.5)]">
       <div className="container mx-auto max-w-7xl px-4 relative">
          ...
          <button 
            ...
            className="absolute -top-6 right-4 bg-[#050505]/95 border border-white/10 border-b-0 rounded-t-lg w-8 h-6 flex items-center justify-center text-gray-500 hover:text-white transition-colors"
            title="Minimize HUD"
          >
